...
export default class Main extends Component {
  ...
  like = target_id => {
    axios
      .post("http://192.168.0.103:1337/v1/like", {
        data: { user_id: this.state.id, target_id: target_id }
      })
  ...
  deslike = target_id => {
    axios
      .post("http://192.168.0.103:1337/v1/deslike", {
        data: { user_id: this.state.id, target_id: target_id }
      })
  ...
  superLike = target_id => {
    axios
      .post("http://192.168.0.103:1337/v1/superLike", {
        data: { user_id: this.state.id, target_id: target_id }
      })
      .then(resp => console.warn(resp)) //Retorna { match : true } caso tenha ocorrido um match.
      .catch(err => console.warn(err));
  ...
  setCards = () => {
    //Inicializa a lista de resultados retornada pelo API.
    axios
      .get("http://192.168.0.103:1337/v1/pairing", {
  ...
  componentDidMount() {
    Geolocation.getCurrentPosition(
      position => {
        ...
        axios
          .put("http://192.168.0.103:1337/v1/callback", {
            data: { lat: latitude, long: longitude, id: this.state.id }
          }) //Grava as coordenadas do usuário no banco.
          .then(resp => {
            this.setCards();
          })
          .catch(err => {
